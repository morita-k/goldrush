...
module BpPicHelper
  ...
  def working_status_type_links_list(bp_pic)
    [working_status_type_link_retired(around_b_if(bp_pic.substitute_bp_pic_id, getShortType("working_status_type","retired")), bp_pic, bp_pic.substitute_bp_pic_id),
     working_status_type_link_changed(around_b_if(bp_pic.change_to_bp_pic_id, getShortType("working_status_type","changed")), bp_pic, bp_pic.change_to_bp_pic_id)]
  end

  def working_status_type_links_show(bp_pic)
    if bp_pic.working?
      [working_status_type_update_link(bp_pic.id, "retired"), working_status_type_update_link(bp_pic.id, "changed")]
    else
...
private
  ...
  def working_status_type_update_link(id, status)
    back_to_link(getLongType("working_status_type",status), {:controller => :bp_pic, :action => :update_working_status_type, :id => id, :working_status_type => status}, :confirm => "勤務ステータスを「#{getLongType("working_status_type",status)}」に変更します。よろしいですか?")
  end
