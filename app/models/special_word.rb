...
class SpecialWord < ActiveRecord::Base
  ...
  def SpecialWord.bp_member_words
    @@bp_member_words ||= get_special_words('bp_member_word').map{|x| Regexp.new(x.target_word, Regexp::IGNORECASE)}
  end

  def SpecialWord.special_words
    @@special_words ||= get_special_words('special_word')
  end

  def SpecialWord.ignore_words
    @@ignore_words ||= get_special_words('ignore_word').map(&:target_word)
  end

  def SpecialWord.ignore_word_propers
    @@bad_proper_words ||= get_special_words('ignore_word_proper').map(&:target_word)
  end

  def SpecialWord.country_words_foreign
    @@country_words_foreign ||= get_special_words('country_word_foreign').map(&:target_word)
  end

  def SpecialWord.get_special_words(specialwordtype)
    require 'zen2han'
    specialword = where(deleted: 0, special_word_type: specialwordtype)
    specialword.each{|x| x.target_word = Zen2Han.toHan(x.target_word).downcase}
  end
end
